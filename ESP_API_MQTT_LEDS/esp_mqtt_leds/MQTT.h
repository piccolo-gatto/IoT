...
void callback(char *topic, byte *payload, unsigned int length) {
    ...
    if (strcmp(topic, "lab/ESP8266_1B9B/strip/set_leds") == 0) {
      int nleds = set_leds(payload, length);
      Serial.print("Set color for leds ");
      Serial.println(nleds);
    }
    if (strcmp(topic, "lab/ESP8266_1B9B/strip/set_leds_bytes") == 0) {
      int nleds = set_leds_bytes(payload, length);
      Serial.print("Set color for leds received in bytes");
      Serial.println(nleds);
    }
    if (strcmp(topic, "lab/ESP8266_1B9B/strip/rotate_leds") == 0) {
      rotate_leds();
    }
    if (strcmp(topic, "lab/ESP8266_1B9B/range") == 0) {
      int power = 1;
      int range = 0;
